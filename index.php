	<!DOCTYPE html>
	<html lang="zxx" class="no-js">
	<head>
		<!-- Mobile Specific Meta -->
		<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
		<!-- Favicon-->
		<link rel="shortcut icon" href="img/favicon.png">
		<!-- Author Meta -->
		<meta name="author" content="codepixer">
		<!-- Meta Description -->
		<meta name="description" content="">
		<!-- Meta Keyword -->
		<meta name="keywords" content="">
		<!-- meta character set -->
		<meta charset="UTF-8">
		<!-- Site Title -->
		<title>ChaiShots</title>

		<link href="https://fonts.googleapis.com/css?family=Poppins:100,200,400,300,500,600,700" rel="stylesheet"> 
			<!--
			CSS
			============================================= -->
			<link rel="stylesheet" href="css/linearicons.css">
			<link rel="stylesheet" href="css/font-awesome.min.css">
			<link rel="stylesheet" href="css/bootstrap.css">
			<link rel="stylesheet" href="css/magnific-popup.css">
			<link rel="stylesheet" href="css/nice-select.css">					
			<link rel="stylesheet" href="css/animate.min.css">
			<link rel="stylesheet" href="css/owl.carousel.css">
			<link rel="stylesheet" href="css/main.css">
		</head>
		<body>

			  <header id="header" id="home">
				<div class="header-top">
		  			<div class="container">
				  		<div class="row justify-content-end">
				  			<div class="col-lg-8 col-sm-4 col-8 header-top-right no-padding">
				  				<ul>
				  					<li>
				  						
				  					</li>				  					
				  				</ul>
				  			</div>
				  		</div>			  					
		  			</div>
				</div>			  	
			    <div class="container">
			    	<div class="row align-items-center justify-content-between d-flex">
				      <div id="logo">
				        <a href="index.php"><img src="img/chai64.png" alt="" title="" /></a>
				      </div>
				      <nav id="nav-menu-container">
				        <ul class="nav-menu">
				          <li class="menu-active"><a href="#home">Home</a></li>
				          <li><a href="#about">About</a></li>
				          <li><a href="#coffee">Products</a></li>
				          <li><a href="#review">Review</a></li>
				          <li><a href="blog.php">Blog</a></li>
				          <li><a href="contact.php">Contact Us</a></li>
				          <li><a href="team.php">Team</a></li>
				          <li><a href="stores.php">Our Stores</a></li>
				          <!-- <li class="menu-has-children"><a href="">Pages</a>
				            <ul>
				              <li><a href="generic.html">Generic</a></li>
				              <li><a href="elements.html">Elements</a></li>
				            </ul>
				          </li> -->
				        </ul>
				      </nav><!-- #nav-menu-container -->		    		
			    	</div>
			    </div>
			  </header><!-- #header -->


			<!-- start banner Area -->
			<section class="banner-area" id="home">	
				<div class="container">
					<div class="row fullscreen d-flex align-items-center justify-content-start">
						<div class="banner-content col-lg-7">
							<!-- <h6 class="text-white text-uppercase">Now you can feel the Energy</h6> -->
							<h1>
								Start your day with <br>
								energetic Chai				
							</h1>
							<a href="#" class="primary-btn text-uppercase">Buy Now</a>
						</div>											
					</div>
				</div>
			</section>
			<!-- End banner Area -->	

			<!-- Start video-sec Area -->
			<section class="video-sec-area pb-100 pt-40" id="about">
				<div class="container">
					<div class="row justify-content-start align-items-center">
						<div class="col-lg-6 video-right justify-content-center align-items-center d-flex">
 							<div class="overlay overlay-bg"></div>
							<a class="play-btn" href="https://www.youtube.com/watch?v=ARA0AxrnHdM"><img class="img-fluid" src="img/play-icon.png" alt=""></a>
						</div>						
						<div class="col-lg-6 video-left">
							<h6>Live Chai making process.</h6>
							<h1>We Telecast our <br>
							Chai Making Live</h1>
							<p><span>We are here to listen from you deliver exellence</span></p>
							<p style="text-align:justify;">
								Philosophy alone cannot alone run a business but passion does, we at Chai Shots firmly believe in the product quality and prudent client service. We are not in the rat race of collecting big data science to serve our customers but imbibing the emotions of taste to the client via our tailor made products and offered services.

								Chai Shots is currently present in Jasola, Kalkaji and Malviya Nagar (New-Delhi)

							</p>
						</div>
					</div>
				</div>	
			</section>
			<!-- End video-sec Area -->
			
			<!-- Start menu Area -->
			<section class="menu-area section-gap" id="coffee">
				<div class="container">
					<div class="row d-flex justify-content-center">
						<div class="menu-content pb-60 col-lg-10">
							<div class="title text-center">
								<h1 class="mb-10">What kind of Products we serve for you</h1>
								<p>Who are in extremely love with eco friendly system.</p>
							</div>
						</div>
					</div>						
					<div class="row">
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Adrak Tulsi Chai</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 39/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Honey Ginger Lemon Chai</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs.49/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Poha</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 79/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>	
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Lemon Ice Tea</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 69/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Oreo Shake</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 89/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Cheesy Bun Omelette</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 59/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Classic Veg Maggie</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 59/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Penne Pasta</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 99/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Green Chutney and Potato Sandwich</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs.69/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Brownie with Ice Cream</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 69/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Pav Bhaji</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 89/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-menu">
								<div class="title-div justify-content-between d-flex">
									<h4>Egg Platter</h4>
									<p class="price float-right">
										<span style="font-family: sans-serif;">Rs. 79/-</span>
									</p>
								</div>
								<p>
									Usage of the Internet is becoming more common due to rapid advance.
								</p>								
							</div>
						</div>																
					</div>
				</div>	
				<div align="center">
					<a href="products.php" class="genric-btn primary circle">Explore more Products<span class="lnr lnr-arrow-right"></span></a>
				</div>
			</section>
			<!-- End menu Area -->
			
			<!-- Start gallery Area --> 
			<section class="gallery-area section-gap" id="gallery">
				<div class="container">
					<div class="row d-flex justify-content-center">
						<div class="menu-content pb-60 col-lg-10">
							<div class="title text-center">
								<h1 class="mb-10">What kind of Products we serve for you</h1>
								<p>Who are in extremely love with eco friendly system.</p>
							</div>
						</div>
					</div>						
					<div class="row">
						<div class="col-lg-4">
							<a href="img/new/Virjin_Mojito.jpg" class="img-pop-home">
								<img class="img-fluid" src="img/new/Virjin_Mojito2.jpg" alt="">
							</a>	
							<a href="img/new/Cheesy_Bun_Omelette.jpg" class="img-pop-home">
								<img class="img-fluid" src="img/new/Cheesy_Bun_Omelette2.jpg" alt="">
							</a>	
						</div>
						<div class="col-lg-8">
							<a href="img/new/Chai_Shots_Thematic.jpg" class="img-pop-home">
								<img class="img-fluid" src="img/new/Chai_Shots_Thematic.jpg" alt="">
							</a>	
							<div class="row">
								<div class="col-lg-6">
									<a href="img/new/Masala_Tea.jpg" class="img-pop-home">
										<img class="img-fluid" src="img/new/Masala_Tea.jpg" alt="">
									</a>	
								</div>
								<div class="col-lg-6">
									<a href="img/new/Masala_Maggie.jpg" class="img-pop-home">
										<img class="img-fluid" src="img/new/Masala_Maggie.jpg" alt="">
									</a>	
								</div>
							</div>
						</div>
					</div>
				</div>	
			</section>
			<!-- End gallery Area -->
			
			<!-- Start review Area -->
			<section class="review-area section-gap" id="review">
				<div class="container">
					<div class="row d-flex justify-content-center">
						<div class="menu-content pb-60 col-lg-10">
							<div class="title text-center">
								<h1 class="mb-10">Reviews From Our Lovely Customers</h1>
								<p>ChaiShots loves to serve you</p>
							</div>
						</div>
					</div>						
					<div class="row">
						<div class="col-lg-6 col-md-6 single-review">
							<!-- <img src="img/r1.png" alt=""> -->
							<div class="title d-flex flex-row">
								<h4>Richa</h4>
								<div class="star">
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star"></span>								
								</div>
							</div>
							<p>
								Loves the Taste of Chai.
							</p>
						</div>	
						<div class="col-lg-6 col-md-6 single-review">
							<!-- <img src="img/r2.png" alt=""> -->
							<div class="title d-flex flex-row">
								<h4>Ankit</h4>
								<div class="star">
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star checked"></span>
									<span class="fa fa-star checked"></span>								
								</div>
							</div>
							<p>
								feel Refresh after having chaishots coffee.
							</p>
						</div>	
					</div>
					<div class="row counter-row">
						<div class="col-lg-3 col-md-6 single-counter">
							<h1>250+</h1>
							<!--<h1 class="counter">250</h1>-->
							<p>Happy Client</p>
						</div>
						<div class="col-lg-3 col-md-6 single-counter">
							<h1>75+</h1>
							<p>Total Products</p>
						</div>
						<div class="col-lg-3 col-md-6 single-counter">
							<h1>2000+</h1>
							<p>Cups Tea</p>
						</div>
						<div class="col-lg-3 col-md-6 single-counter">
							<h1>4+</h1>
							<p>Total Stores</p>
						</div>																		
					</div>
				</div>	
			</section>
			<!-- End review Area -->
			
			<!-- Start blog Area -->
		<!--	<section class="blog-area section-gap" id="blog">
				<div class="container">
					<div class="row d-flex justify-content-center">
						<div class="menu-content pb-60 col-lg-10">
							<div class="title text-center">
								<h1 class="mb-10">What kind of Products we serve for you</h1>
								<p>Who are in extremely love with eco friendly system.</p>
							</div>
						</div>
					</div>						
					<div class="row">
						<div class="col-lg-6 col-md-6 single-blog">
							<img class="img-fluid" src="img/b1.jpg" alt="">
							<ul class="post-tags">
								<li><a href="#">Travel</a></li>
								<li><a href="#">Life Style</a></li>
							</ul>
							<a href="#"><h4>Portable latest Fashion for young women</h4></a>
							<p>
								Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore  et dolore.
							</p>
							<p class="post-date">
								31st January, 2018
							</p>
						</div>
						<div class="col-lg-6 col-md-6 single-blog">
							<img class="img-fluid" src="img/b2.jpg" alt="">
							<ul class="post-tags">
								<li><a href="#">Travel</a></li>
								<li><a href="#">Life Style</a></li>
							</ul>
							<a href="#"><h4>Portable latest Fashion for young women</h4></a>
							<p>
								Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore  et dolore.
							</p>
							<p class="post-date">
								31st January, 2018
							</p>
						</div>						
					</div>
				</div>	
			</section> -->
			<!-- End blog Area -->
			<?php include 'includes/footer.php'; ?>

		</body>
	</html>



